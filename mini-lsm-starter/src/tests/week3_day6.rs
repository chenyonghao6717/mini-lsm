...
fn test_serializable_1() {
    ...
    let _ = txn1.put(b"key1", &txn1.get(b"key2").unwrap().unwrap());
    let _ = txn2.put(b"key2", &txn2.get(b"key1").unwrap().unwrap());
    txn1.commit().unwrap();
...
fn test_serializable_2() {
    ...
    let txn1 = storage.new_txn().unwrap();
    let txn2 = storage.new_txn().unwrap();
    let _ = txn1.put(b"key1", b"1");
    let _ = txn2.put(b"key1", b"2");
    txn1.commit().unwrap();
    txn2.commit().unwrap();
    assert_eq!(storage.get(b"key1").unwrap(), Some(Bytes::from("2")));
}
