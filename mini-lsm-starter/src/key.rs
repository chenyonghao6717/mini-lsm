...
pub const TS_SIZE: usize = size_of::<u64>();
...
impl Key<Vec<u8>> {
    ...
    pub fn from_raw_vec(mut key: Vec<u8>) -> Self {
        assert!(key.len() >= TS_SIZE);
        let raw_ts = key.split_off(key.len() - TS_SIZE);
        let ts = u64::from_le_bytes([
            raw_ts[0], raw_ts[1], raw_ts[2], raw_ts[3], raw_ts[4], raw_ts[5], raw_ts[6], raw_ts[7],
        ]);
        Self::from_vec_with_ts(key, ts)
    }
...
impl Key<Bytes> {
    ...
    pub fn from_bytes(bytes: Bytes) -> KeyBytes {
        assert!(bytes.len() >= TS_SIZE);
        let key = bytes.slice(..bytes.len() - TS_SIZE);
        let raw_ts = bytes.slice(bytes.len() - TS_SIZE..);
        let ts = u64::from_le_bytes([
            raw_ts[0], raw_ts[1], raw_ts[2], raw_ts[3], raw_ts[4], raw_ts[5], raw_ts[6], raw_ts[7],
        ]);
        Self::from_bytes_with_ts(key, ts)
    }
...
impl<'a> Key<&'a [u8]> {
    ...
    pub fn from_raw_slice(raw_slice: &'a [u8]) -> Self {
        assert!(raw_slice.len() >= TS_SIZE);
        let key = &raw_slice[..raw_slice.len() - TS_SIZE];
        let raw_ts = &raw_slice[raw_slice.len() - TS_SIZE..];
        let ts = u64::from_le_bytes([
            raw_ts[0], raw_ts[1], raw_ts[2], raw_ts[3], raw_ts[4], raw_ts[5], raw_ts[6], raw_ts[7],
        ]);
        Self::from_slice(key, ts)
    }
